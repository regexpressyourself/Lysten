...
void handle_client(int client_sockfd) {
    ...
    switch(pid) {
        ...
        case 0: // child
            ...
            close(master_pty_fd);
            close(client_sockfd);
            if(setup_slave_pty(slave_pty_name) < 0) {
                pid = getppid();
                kill(pid, SIGTERM);
                exit(1);
            }
        ...
        default: // parent
            ...
            read_write_return_val = handle_pty_master(client_sockfd, master_pty_fd);
...
int handle_pty_master(int client_sockfd, int master_pty_fd) 
{
    ...
    switch (pid) {
        ...
        case 0: // child
            while (1){
                // read from stdin
                read_write_return_val = from_client_to_pty(client_sockfd, master_pty_fd);
                if (read_write_return_val < 0) {
                    close(client_sockfd);
                    close(master_pty_fd);
                    exit(1);
        ...
        default:
            while (1){
                ...
                read_write_return_val = from_pty_to_client(client_sockfd, master_pty_fd);
                if (read_write_return_val < 0) {
                    close(client_sockfd);
                    close(master_pty_fd);
                    kill(pid, SIGTERM);
...
int from_pty_to_client(int client_sockfd, int master_pty_fd) {
    ...
    if (bytes_read < 0) {
        printf("read from masterpty failed\n");
        return -1;
    }
